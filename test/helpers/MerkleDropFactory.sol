...
import {MerkleLib} from "test/helpers/MerkleLib.sol";
...
contract MerkleDropFactory {
  ...
  struct MerkleTree {
    ...
    uint256 tokenBalance; // amount of tokens allocated for this tree
    uint256 spentTokens; // amount of tokens dispensed from this tree
  ...
  event MerkleTreeAdded(uint256 indexed treeIndex, address indexed tokenAddress, bytes32 newRoot, bytes32 ipfsHash);

  // every time a tree is topped up
  event TokensDeposited(uint256 indexed treeIndex, address indexed tokenAddress, uint256 amount);

  error BadTreeIndex(uint256 treeIndex);
  error LeafAlreadyClaimed(uint256 treeIndex, bytes32 leafHash);
  error BadProof(uint256 treeIndex, bytes32 leaf, bytes32[] proof);
  error TokensNotTransferred(uint256 treeIndex, bytes32 leaf);
  ...
  /// @param ipfsHash the ipfs hash of the entire dataset, used for redundance so that creator can ensure merkleproof
  /// are always computable
  /// @param tokenAddress the address of the token contract that is being distributed
  /// @param tokenBalance the amount of tokens user wishes to use to fund the airdrop, note trees can be
  /// under/overfunded
  function addMerkleTree(bytes32 newRoot, bytes32 ipfsHash, address tokenAddress, uint256 tokenBalance) public {
  ...
  /// @dev Anyone may call this function, the only risk here is that the token contract is malicious, rendering the tree
  /// malicious
  ...
  function depositTokens(uint256 treeIndex, uint256 value) public {
  ...
  function withdraw(uint256 treeIndex, address destination, uint256 value, bytes32[] memory proof) public {
  ...
  function getWithdrawn(uint256 treeIndex, bytes32 leaf) external view returns (bool) {
