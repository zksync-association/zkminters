...
// Read the bytecode hash from ZkMinterTriggerV1.json
// Verify the zksolc version used to compile the contract, the hash changes with different versions
const zkMinterTriggerV1Path = path.join(
  __dirname,
  "../artifacts-zk/src/ZkMinterTriggerV1.sol/ZkMinterTriggerV1.json"
);
const zkMinterTriggerV1Json = JSON.parse(
  fs.readFileSync(zkMinterTriggerV1Path, "utf8")
);
// Extract the bytecode from the hardhat artifacts
const bytecode = zkMinterTriggerV1Json.bytecode;
...
async function main() {
  ...
  const contractName = "ZkMinterTriggerV1Factory";
